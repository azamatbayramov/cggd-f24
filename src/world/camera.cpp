...
{
	theta = in_theta * static_cast<float>(M_PI) / 180.0f;
}

void cg::world::camera::set_phi(float in_phi)
{
	phi = in_phi * static_cast<float>(M_PI) / 180.0f;
}

void cg::world::camera::set_angle_of_view(float in_aov)
{
	angle_of_view = in_aov * static_cast<float>(M_PI) / 180.0f;
}
...
{
	float f = 1.0f / std::tanf(angle_of_view * 0.5f);
