...
namespace cg::renderer
{
			...
			const triangle<VB>& triangle, const ray& ray) const
	{
		...
		payload.bary = float3{1.0f - v - u, u, v};
	...
	float2 raytracer<VB, RT>::get_jitter(int frame_id)
	{
		...
		float inv_base = 1.0f / static_cast<float>(base_x);
		...
		index = frame_id + 1;
		inv_base = 1.0f / static_cast<float>(base_y);
		fraction = inv_base;
	...
	{
		float3 inv_ray_dir = float3(1.0f) / ray.direction;
