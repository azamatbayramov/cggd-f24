...
namespace cg
{
	...
	struct unsigned_color
	{
		static unsigned_color from_color(const color& color)
		{
			...
			out.r = std::clamp(static_cast<int>(color.r * 255.0f), 0, 255);
			out.g = std::clamp(static_cast<int>(color.g * 255.0f), 0, 255);
			out.b = std::clamp(static_cast<int>(color.b * 255.0f), 0, 255);
